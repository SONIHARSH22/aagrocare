    ...
    const savedPlants1 = JSON.parse(localStorage.getItem('communityplants')) || [];
    ...
    function addPlantWithCustomName() {
        ...
        const uploadedImage = plantImageInput.files[0];
        ...
        reader.onload = function () {
            const imageSrc = reader.result; // Data URL
            createPlantDiv(name, imageSrc);
            hideModal();

            savedPlants1.push({ name, imageSrc });
            localStorage.setItem('communityplants', JSON.stringify(savedPlants1));
        };
        reader.readAsDataURL(uploadedImage);
    }

    function createPlantDiv(name, imageSrc) {
        ...
        const profdiv = document.createElement('div');
            ...
            profdiv.innerHTML = ` <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" class="bi bi-person-circle" viewBox="0 0 16 16">
        ...
        p.textContent = name;
         ...
         // Append the like, dislike, chat, and WhatsApp icons
         const lcsDiv = document.createElement('div');
            lcsDiv.className = 'lcs';
          ...
          <svg xmlns="http://www.w3.org/2000/svg" width="23" height="23" fill="green" class="bi bi-whatsapp" viewBox="0 0 16 16">
    ...
    savedPlants1.forEach((plantData) => {
        const { name, imageSrc } = plantData;
        createPlantDiv(name, imageSrc);
    });

    addPlantButton.addEventListener('click', showModal);
